...
class MailService:
    ...
    def nofity_event_owner(self, event: Event):
        data = {
            "To": [{"Email": event.owner.email, "Name": event.owner.username}],
            "Subject": f"Event {event.name} statistics",
            "HTMLPart": f"<h3>Hello dear {event.owner.username}, there are some statistics about event #{event.pk} created by you! </h3><br /><h4>Total questions: {len(event.questions)}.</h4><br /><h4>Questions for last week: {len(event.questions)}</h4>",  # noqa # TODO: count of questions for last week
            "CustomID": "AppGettingStartedTest",
        }

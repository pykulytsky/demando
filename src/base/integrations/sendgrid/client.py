...
class SendgridApp:
    def __init__(
        self, db: Depends(get_db), token: str = settings.SENDGRID_API_KEY
    ) -> None:
    ...
    async def _send_email(
        self, user_pk: int, template_id: int, subject: str, dynamic_template_data: dict
    ):
    ...
    async def send_verification_mail(self, user_pk: int):
        ...
        return await self._send_email(
            user_pk, template_id, subject, dynamic_template_data
        )
